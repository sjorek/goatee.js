...
exports.Context = class Context
  ...
  #                       instantiated. (@see implementation of select-action)
  ...
  # alter action expressions, variable names must start with $, but depending
  ...
  evaluate: (expression, template) ->
    fn = (_evaluate ? _evaluate = Expression.get().evaluateToFunction) expression
    @execute fn, template
  ...
  # @param {goatee.Action.Context|null} parent
  # @return {goatee.Action.Context}
  @create: (data, parent) ->
