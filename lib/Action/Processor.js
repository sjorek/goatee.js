...
(function() {
  var ATT_cache, DEBUG, PROP_cache, Processor, STRING_assigment, STRING_empty, STRING_seperator, STRING_zero, bind, exports, getAttribute, ownerDocument, setAttribute, traverseElements, _ref, _ref1, _ref2;

  _ref = require('../Core/Constants').Constants, DEBUG = _ref.DEBUG, ATT_cache = _ref.ATT_cache, PROP_cache = _ref.PROP_cache, STRING_assigment = _ref.STRING_assigment, STRING_empty = _ref.STRING_empty, STRING_seperator = _ref.STRING_seperator, STRING_zero = _ref.STRING_zero;

  _ref1 = require('../Dom/Document').Document, getAttribute = _ref1.getAttribute, setAttribute = _ref1.setAttribute, traverseElements = _ref1.traverseElements, ownerDocument = _ref1.ownerDocument;

  bind = require('../Core/Utility').Utility.bind;
  ...
  exports.Processor = Processor = (function() {
    ...
    Processor.prototype.run = function(fn) {
      var arg1, arg2, arrays, calls, index, indices, method, queue;
      this.calls = calls = [];
    ...
    Processor.prototype.setup = function(template) {
      ...
      if (this.getCacheProperty(template) == null) {
        self = this;
        Document.traverseElements(template, function(node) {
    ...
    Processor.prototype.prepare = function(node) {
      ...
      id = this.getCacheIdentifier(node);
      if (id != null) {
      ...
      source = this.flatten(_list);
      id = this.getSourceIdentifier(source);
      if (id != null) {
        cache = this.getCache(id);
        if (cache != null) {
          this.setCacheIdentifier(node, id);
          return this.setCacheProperty(node, cache);
        }
      }
      cache = this.createCache(node, _values);
    ...
    Processor.prototype.collect = function(node, list, values) {
      var name, value, _i, _len, _ref3;
      _ref3 = Processor.actions;
      for (_i = 0, _len = _ref3.length; _i < _len; _i++) {
        name = _ref3[_i][0];
        value = Document.getAttribute(node, name);
        values[name] = value;
        if (value != null) {
          list.push(this.combine(name, value));
        }
      }
    };

    Processor.prototype.combine = function(name, value) {
      return Constants.STRING_empty + name + Constants.STRING_assigment + value;
    };

    Processor.prototype.flatten = function(list) {
      return list.join(Constants.STRING_seperator);
    };

    Processor.prototype.createCache = function(node, values) {
      var cache, name, parse, value, _debugCache, _i, _len, _ref3, _ref4, _ref5;
      cache = {};
      _ref3 = Processor.actions;
      for (_i = 0, _len = _ref3.length; _i < _len; _i++) {
        _ref4 = _ref3[_i], name = _ref4[0], parse = _ref4[1];
        value = values[name];
        if (value == null) {
          continue;
        }
        ...
        if (Constants.DEBUG) {
          _debugCache = (_ref5 = cache._debugCache) != null ? _ref5 : cache._debugCache = {};
    ...
    Processor.prototype.getCacheProperty = function(node) {
      return node[Constants.PROP_cache];
    };

    Processor.prototype.setCacheProperty = function(node, actions) {
      return node[Constants.PROP_cache] = actions;
    };
    ...
    Processor.prototype.getCacheIdentifier = function(node) {
      return Document.getAttribute(node, Constants.ATT_cache);
    };

    Processor.prototype.setCacheIdentifier = function(node, id) {
    ...
    Processor.prototype.setEmpty = function(node) {
      this.setCacheIdentifier(node, Constants.STRING_zero);
